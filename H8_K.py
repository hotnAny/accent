...
Barr = array(B.subs({a:_a, b:_b, c:_c, x:0, y:0, z:0})).astype('double')
Barr.dump(b_fname)
Carr = array(C.subs({nu:_nu, g:_g, G:_G})).astype('double')
Carr.dump(c_fname)
...
def getB(_x, _y, _z):
    global B
    return B.subs({a:_a, b:_b, c:_c, x:_x, y:_y, z:_z})

def getC():
    global C
    return C.subs({nu:_nu, g:_g, G:_G})
